...
const pageParams = (pagination, params) => {
    ...
    const maxResults = Math.max(
        +params.maxResults || +params.pageSize || +_pagination.maxResults,
        0
    );
    const startIndex = params.startIndex
        ? Math.max(+params.startIndex || 0, 0)
        : params.page
        ? (Math.max(+params.page || 1, 0) - 1) * maxResults
        : 0;
    return {
...
const findAllOpts = (Model, params, isAll) => {
    ...
    if (_.isString(params["keyword"]) && params["keyword"] != "") {
        ...
        let orWhere = [];
        _.each(
            Model.filterAttrs || _.keys(Model.rawAttributes),
            function (name) {
                ...
                if (
                    ["DATE", "DATETIME"].indexOf(fieldType) < 0 &&
                    name != "id"
                ) {
    ...
    if (includes) {
        _.each(includes, (x) => {
            const includeWhere = {};
            const filterAttrs =
                x.model.filterAttrs || _.keys(x.model.rawAttributes);
