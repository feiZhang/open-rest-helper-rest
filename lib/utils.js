...
var findAllOpts = function(Model, params, isAll) {
  ...
  const getKeywordWhere = (haveDate, params, Model) => {
    if (_.isString(params['keyword']) && params['keyword'] != "") {
      const keyword = '%' + params['keyword'].replace(" ","%").replace("　","%") + '%';
      var orWhere = [];
      _.each(Model.filterAttrs || _.keys(Model.rawAttributes), function (name) {
        const fieldType = Model.rawAttributes[name].type.toString();
        var oneWhere = {};
        if (fieldType == 'DATETIME') {
          if (haveDate) {// || (name != 'createdAt' && name != "updatedAt")){
            //在进行includes联结时，下面的条件没有表别名，SQL语句执行错误。暂时没有找到带上表别名的方法。。不优雅的先处理掉。
            oneWhere = sequelize.where(sequelize.fn('date_format', sequelize.col(name), '%Y-%m-%d %h:%i:%s'), "like", keyword)
            orWhere.push(oneWhere);
          }
        } else {
          oneWhere[name] = { $like: keyword }
          orWhere.push(oneWhere);
        }
      });
      return orWhere;
  ...
  if (!!params && !!_.isObject(params)) {
    //关键词模糊检索
    const orWhere = getKeywordWhere(!includes, params, Model);
    if (orWhere !== false) where.$or = orWhere;
    _.each(Model.filterAttrs || _.keys(Model.rawAttributes), function(name) {
  ...
  if (includes) {
    _.each(includes, function(x) {
      var includeWhere = {};
      if (params[x.as]) {
        const orWhere = getKeywordWhere(false, params[x.as], x.model);
        if (orWhere !== false) includeWhere.$or = orWhere;
...
var pageParams = function(pagination, params) {
  ...
  var page = Math.max((+params.page || 1), 0);
...
var findOptFilter = function(params, name, where, col) {
  ...
  // 处理where in
  if (_.isString(params[name + 's']) && params[name + 's'].trim() != "") {
    if (!where[col]) where[col] = {};
...
var pickParams = function(req, cols, Model) {
  ...
  }else if (Model.onlyManagerCols && (req.isManager !== true)) {
    cols = _.filter(cols, function(x) {
      return !_.includes(Model.onlyManagerCols, x);
    });
  }
