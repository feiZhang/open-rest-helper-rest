...
const pageParams = (pagination, params) => {
  ...
  const startIndex = params.startIndex ? Math.max((+params.startIndex || 0), 0) : (params.page ? (Math.max((+params.page || 1), 0) - 1) * maxResults : 0);
  return {
...
const sort = (params, conf) => {
  ...
  if (!value) {
    if (Array.isArray(conf.default)) return conf.default;
...
// findOptFilter 的处理
const findOptFilter = (Op, params, name, where, col = name) => {
  let value;
  if (!params) return;
...
const findAllOpts = (Model, params, isAll) => {
  ...
  _.each(Model.filterAttrs || _.keys(Model.rawAttributes), (name) => {
    findOptFilter(Model.sequelize.Op, params, name, where);
  });
  ...
  if (_.isString(params['keyword']) && params['keyword'] != "") {
    // const keyword = `%${mysql.escape(params['keyword']).replace(" ", "%").replace("　", "%")}%`;
    const keyword = `%${params['keyword'].replace(' ', '%').replace('　', '%')}%`;
    let orWhere = [];
    _.each(Model.filterAttrs || _.keys(Model.rawAttributes), function (name) {
      const fieldType = Model.rawAttributes[name].type.toString({ escape: mysql.escape });
      let oneWhere = {};
      // console.log(fieldType);
      if (['DATE', 'DATETIME'].indexOf(fieldType) < 0 && name != 'id') {
        oneWhere[name] = { [Model.sequelize.Op.like]: keyword }
        orWhere.push(oneWhere);
      }
    });
    // console.log(where, orWhere);
    if (orWhere.length > 0) where.$or = orWhere;
  }
  ...
  if (includes) {
    _.each(includes, (x) => {
      ...
      _.each(filterAttrs, (name) => {
        findOptFilter(Model.sequelize.Op, params[x.as], name, includeWhere, name);
      });
